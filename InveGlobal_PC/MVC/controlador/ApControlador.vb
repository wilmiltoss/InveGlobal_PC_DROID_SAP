...
#Region "ATRIBUTOS"
    ...
    Public Const VERSION As String = "8.5.7"
...
#Region "FORMULARIOS"
    ...
    Private __AbmSoporte_inventarioFRM As frm_inventario_soporte
    Private __AbmImportar_conteoFRM As frm_importar_carga
    Private __Detalles_Error As frm_detalle_error

    Private __SeleccionarServidor As frm_servidor
...
#Region "METODOS"
    ...
    End Function
    ''' <summary>
    ''' Recupera los items de existencia desde el Sist. SAP para el local actual
    ''' </summary>
    ''' <returns>Lista de Resultados [int, object]</returns>
    ''' <remarks></remarks>
    ...
    End Function
    ''' <summary>
    ''' Importamos el archivo desde un cuadro de dialogo para cargarlo a la lista
    ''' </summary>
    ...
    Public Function lTransferir_parametros_a_colector() As List(Of Object)
        ...
        Try
            ...
            If lResultado(0).Equals(1) Then
                ...
                oConfig.nIdInventario = Inventario_OTD.nId
                oConfig.nIpSqlServer = cServidorSQL
                ...
                'transferimos la tabla inventario_soporte
                __lTransferir_inventario_soporte_sqlite()
            ...
            If lResultado(0).Equals(1) Then
                ...
                For Each oSoporte As SoporteOTD In lSoportes
                    ...
                    Principal_frm().mostrar_mensaje_estado("Cargando soporte " _
                                                           , String.Format("{0} de {1}", c, lSoportes.Count)
                                                           )
                    Principal_frm.pbr_progreso.Value = c
    ...
    ''' <summary>
    ''' Ejecuta la transferencia de datos de inventario_soporte a la BD SQLite
    ''' </summary>
    ''' <returns>Lista de Resultados [int, object]</returns>
    ''' <remarks></remarks>
    Private Function __lTransferir_inventario_soporte_sqlite() As List(Of Object)

        Dim NOMBRE_METODO As String = NOMBRE_CLASE & ".__lTransferir_inventario_soporte_sqlite()"
        Dim lResultado As New List(Of Object)(New Object() {0, NOMBRE_METODO & " No Ejecutado!"})

        Try
            'mensaje de estado
            Principal_frm().mostrar_mensaje_estado("Pasando parametros de ", " inventario_soporte")

            Dim aFiltroWhere As String = String.Format(" Where {0} = {1}", InventarioSoportesTBL.ID_INVENTARIO.cNombre, Inventario_OTD.nId)
            'recuperamos los soportes
            lResultado = oApModelo.InventarioSoportesADM.lGet_elementos(aFiltroWhere)

            'si se ejecuto sin problemas
            If lResultado(0).Equals(1) Then
                'tomamos la lista devuelta
                Dim lInventarioSoportes As List(Of InventarioSoporteOTD) = CType(lResultado(1), List(Of InventarioSoporteOTD))

                'si tiene elementos, llamamos al metodo de limpieza de la tabla
                lResultado = oApModeloSQLite.InventarioSoporte_ADM().lEliminar(New InventarioSoporteSQLiteOTD())

                'estado
                Dim c As Integer = 0
                Principal_frm.pbr_progreso.Value = 0
                Principal_frm.pbr_progreso.Maximum = lInventarioSoportes.Count

                'recorremos la lista y las insertamos en la tabla nueva
                For Each oInventarioSoporte As InventarioSoporteOTD In lInventarioSoportes
                    'mensaje de estado
                    Principal_frm().mostrar_mensaje_estado("Cargando soporte " _
                                                           , String.Format("{0} de {1}", c, lInventarioSoportes.Count)
                                                           )
                    Principal_frm.pbr_progreso.Value = c

                    'creamos una nueva instancia de soporte
                    Dim oInventarioSoporteSQLite As New InventarioSoporteSQLiteOTD()
                    oInventarioSoporteSQLite.nIdInventarioSoporte = oInventarioSoporte.nIdInventarioSoporte
                    oInventarioSoporteSQLite.nIdInventario = oInventarioSoporte.nIdInventario
                    oInventarioSoporteSQLite.nIdClave = oInventarioSoporte.nIdClave
                    oInventarioSoporteSQLite.nIdSoporte = oInventarioSoporte.nIdSoporte
                    oInventarioSoporteSQLite.nNroSoporte = oInventarioSoporte.nNroSoporte
                    oInventarioSoporteSQLite.nNroTipoSoporte = oInventarioSoporte.nNroTipoSoporte
                    oInventarioSoporteSQLite.cDescripcionSoporte = oInventarioSoporte.cDescripcionSoporte
                    oInventarioSoporteSQLite.nConteo1 = oInventarioSoporte.nConteo1
                    oInventarioSoporteSQLite.nConteo2 = oInventarioSoporte.nConteo2
                    oInventarioSoporteSQLite.nConteo3 = oInventarioSoporte.nConteo3
                    oInventarioSoporteSQLite.nIdLocaciones = oInventarioSoporte.nIdLocaciones

                    'llamamos al metodo de insercion en la tabla sqlite
                    lResultado = oApModeloSQLite.InventarioSoporte_ADM().lAgregar(oInventarioSoporteSQLite)

                    'si no se ejecuto correctamente
                    If Not lResultado(0).Equals(1) Then
                        'evento a log
                        log().Escribir(NOMBRE_METODO & " Error insertando Inventario_Soporte en SQLite: " & lResultado(1).ToString())
                        'salimos del bucle
                        Exit For
                    End If
            ...
            Else
                'si no se recuperaron las locaciones, evento alog
                log().Escribir(NOMBRE_METODO & " Error recuperando Inventario_Soporte: " & lResultado(1).ToString())
    ...
    Public Function lTransferir_bd_a_colector() As List(Of Object)
        ...
        Try
            ...
            If __PrincipalFRM.mnu_modo_transmision.SelectedIndex <= 0 Then
                ...
                cDestinoSQLite += Config.Get_instancia().get_valor(Config.NOMBRE_SQLITE)
    ...
    ''' <summary>
    ''' Despliega el mensaje de evento para seleccionar cierre de aplicacion
    ''' </summary>
    ''' <param name="cMensaje">Mensaje a desplegar</param>
    ''' <param name="cTitulo">Titulo del cuadro de mensajes</param>
    ''' <remarks></remarks>
    Public Sub notificar_mensaje_cierre(ByVal cMensaje As String, ByVal cTitulo As String)
        Dim opcion As DialogResult
        'mostramos el mensaje de notificacion
        opcion = MessageBox.Show(cMensaje, cTitulo, MessageBoxButtons.YesNo, MessageBoxIcon.Information)
        If (opcion = DialogResult.Yes) Then
            __PrincipalFRM.Close()
        End If
...
#End Region
    ...
    ''' <summary>
    ''' Devuelve una instancia del formulario de ABM de Soportes de inventarios
    ''' </summary>
    ''' <value></value>
    ''' <returns>Instancia del formulario de ABM de Soportes de inventarios</returns>
    ''' <remarks></remarks>
    Public ReadOnly Property Get_abm_soporte_inventario_frm() As frm_inventario_soporte
        Get
            If __AbmSoporte_inventarioFRM Is Nothing Then
                __AbmSoporte_inventarioFRM = New frm_inventario_soporte(Me)
            ElseIf __AbmSoporte_inventarioFRM.IsDisposed Then
                __AbmSoporte_inventarioFRM = New frm_inventario_soporte(Me)
            End If
            Return __AbmSoporte_inventarioFRM
        End Get
    End Property

    ''' <summary>
    ''' Devuelve una instancia del formulario de ABM de Importacion de conteos
    ''' </summary>
    ''' <value></value>
    ''' <returns>Instancia del formulario de ABM de Importacion de conteos</returns>
    ''' <remarks></remarks>
    Public ReadOnly Property Get_abm_importar_conteo_frm() As frm_importar_carga
        Get
            If __AbmImportar_conteoFRM Is Nothing Then
                __AbmImportar_conteoFRM = New frm_importar_carga(Me)
            ElseIf __AbmImportar_conteoFRM.IsDisposed Then
                __AbmImportar_conteoFRM = New frm_importar_carga(Me)
            End If
            Return __AbmImportar_conteoFRM
        End Get
    End Property


    ''' <summary>
    ''' Devuelve la referencia a la instancia del formulario de seleccion de servidor
    ''' </summary>
    ''' <value></value>
    ''' <returns>Referencia a la instancia de FrmServidor</returns>
    ''' <remarks></remarks>
    Public ReadOnly Property Get_servidor_frm() As frm_servidor
        Get
            If __SeleccionarServidor Is Nothing Then
                __SeleccionarServidor = New frm_servidor(Me)
            End If
            Return __SeleccionarServidor
        End Get
    End Property
...
#End Region
    ...
    'Funcion transferir maestro por wifi
    Public Function lTransferir_bd_a_colector_Android_FTP() As List(Of Object)
        Dim NOMBRE_METODO As String = NOMBRE_CLASE & ".lTransferir_bd_a_colector_Android_FTP()"
        Dim lResultado As New List(Of Object)(New Object() {0, NOMBRE_METODO & " No Ejecutado!"})
        Try
            lResultado = oApModeloSQLite.Get_conector().lConectar() 'conecta a la bd
            'si se pudo conectar
            If lResultado(0).Equals(1) Then
                'llamamos al metodo de creacion de la tabla LECTURAS
                lResultado = oApModeloSQLite.Lecturas_ADM().lCrear_tabla()
            ...
            lResultado = oApModeloSQLite.Get_conector().lDesconectar(NOMBRE_METODO)
            'path de destino del archivo
            '''//////DE PASA EL ARCHIVO POR MEDIO DE LA CONEXION FTP WIFI/////////////////////////
            Dim destino As String = "ftp://" & FrmPrincipal.Txt_Ip.Text & "/Download/" + Config.Get_instancia().get_valor(Config.NOMBRE_SQLITE) 'copia en la siguiente direccion con el nombre indicado
            Dim miRequest As Net.FtpWebRequest = Net.WebRequest.Create(destino)
            miRequest.Credentials = New Net.NetworkCredential("android", "android") 'usuario y contraseña de la ip
            miRequest.Method = Net.WebRequestMethods.Ftp.UploadFile
            Try
                Dim origen() As Byte = System.IO.File.ReadAllBytes(oApModeloSQLite.Get_path_sqlite()) 'directorio de donde proviene el archivo
                Dim miStream As System.IO.Stream = miRequest.GetRequestStream()
                miStream.Write(origen, 0, origen.Length)
                miStream.Close()
                miStream.Dispose()
            Catch ex As Exception
            ...
            End Try
            'intentamos transferir el archivo de la PC al colector
            'si se ejecuto sin problemas_________________________________________________________________________________________________
            If lResultado(0).Equals(1) Then
                'mensaje de notificacion
                notificar_exito("Archivo Transferido Existosamente!", "Maestro a Colector")
            Else
    ...
    'Funcion transferir maestro por usb
    Public Function lTransferir_bd_a_colector_Android_USB() As List(Of Object)
        Dim NOMBRE_METODO As String = NOMBRE_CLASE & ".lTransferir_bd_a_colector_Android_USB()"
        Dim lResultado As New List(Of Object)(New Object() {0, NOMBRE_METODO & " No Ejecutado!"})
        Try
            lResultado = oApModeloSQLite.Get_conector().lConectar() 'conecta a la bd
            'si se pudo conectar
            ...
            lResultado = oApModeloSQLite.Get_conector().lDesconectar(NOMBRE_METODO)
            'path de destino del archivo
            '''//////DE PASA EL ARCHIVO POR MEDIO DE LA CONEXION USB/////////////////////////

            Dim destino As String = "C:\docu\" + Config.Get_instancia().get_valor(Config.NOMBRE_SQLITE) 'copia en la siguiente direccion backup temporal
            'Dim miRequest As Net.FtpWebRequest = Net.WebRequest.Create(destino) ' si no hay, lo crea
            'miRequest.Method = Net.WebRequestMethods.Ftp.UploadFile
            Dim origen As String = oApModeloSQLite.Get_path_sqlite() 'directorio de donde proviene el archivo
            Try
                My.Computer.FileSystem.CopyFile(origen, destino)

            Catch ex As Exception
                Throw New Exception(ex.Message & ". El Archivo no pudo ser copiado a temporal.")
            End Try
            'intentamos transferir el archivo de la PC al colector
            'si se ejecuto sin problemas_________________________________________________________________________________________________
            If lResultado(0).Equals(1) Then
    ...
    '**************************************BORRADOR*******************************************************************
    Public Function lTransferir_bd_a_colector_Android_() As List(Of Object)
        Dim NOMBRE_METODO As String = NOMBRE_CLASE & ".lTransferir_bd_a_colector_Android_USB()"
        Dim lResultado As New List(Of Object)(New Object() {0, NOMBRE_METODO & " No Ejecutado!"})
        Try
            'si el equipo esta conectado
            ...
            'path de destino del archivo
            '''/////////////// DE PASA EL ARCHIVO POR MEDIO DE LA CONEXION USB ////////////////////////////
            Dim seleccionarCarpeta As New SaveFileDialog
            seleccionarCarpeta.Title = "Guardar en...."
            seleccionarCarpeta.Filter = "SQLITE|*.sqlite"
            seleccionarCarpeta.FileName = Config.Get_instancia().get_valor(Config.NOMBRE_SQLITE) ' pone el nombre
            Dim cDestinoSQLiteAndroid As String
            seleccionarCarpeta.ShowDialog() 'ruta y nombre del fichero
            cDestinoSQLiteAndroid = seleccionarCarpeta.FileName
            Dim destinofinal As String
            destinofinal = cDestinoSQLiteAndroid
            'intentamos transferir el archivo de la PC al colector
            lResultado = androidMobile.lMover_Archivo_Dispositivo1(oApModeloSQLite.Get_path_sqlite(), cDestinoSQLiteAndroid, True, True)
            'elimina el existente
            My.Computer.FileSystem.DeleteFile(cDestinoSQLiteAndroid)
            'copia el archivo origen al archivo destino
            My.Computer.FileSystem.CopyFile(oApModeloSQLite.Get_path_sqlite(), 'archivo origen
            destinofinal, FileIO.UIOption.AllDialogs) 'archivo destino
            'intentamos transferir el archivo de la PC al colector_______________________________________________________________________
            'si se ejecuto sin problemas_________________________________________________________________________________________________
            If lResultado(0).Equals(1) Then
                'mensaje de notificacion
                notificar_exito("Archivo Transferido Existosamente!", "Maestro a Colector")
